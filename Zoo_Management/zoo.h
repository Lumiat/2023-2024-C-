...
class Animal_Manage{
    ...
    //纯虚函数
    virtual void add_animal()=0;//管理员权限：添加动物
    virtual void rmv_animal()=0;//管理员权限：移除动物
    virtual void health_check() const =0;//管理员+饲养员权限：健康检查
    virtual void feed_check() const =0;//管理员+饲养员权限：饲喂
    virtual void show_park_species() const =0;//管理员+饲养员+游客：查看某园区有什么动物种群
    virtual void show_keeper() const =0;//管理员权限：显示某个种群的饲养员
    virtual void show_animal_info() const =0;//游客权限：显示某种动物的分类、种群数量、食物;对管理员+饲养员权限：显示种群数量
    virtual void show_environment() const =0;//管理员+饲养员+游客：显示某园区环境
};
...
class Aqua_Area:public Animal_Manage{
    ...
    void add_animal();//管理员权限：添加动物
    void rmv_animal();//管理员权限：移除动物
    void health_check() const ;//管理员+饲养员权限：健康检查
    void feed_check() const ;//管理员+饲养员权限：饲喂
    void show_park_species() const ;//管理员+饲养员+游客：查看某园区有什么动物种群
    void show_keeper() const ;//管理员权限：显示某个种群的饲养员
    void show_animal_info() const ;//游客权限：显示某种动物的分类、种群数量、食物;对管理员+饲养员：显示种群数量
    void show_environment() const ;//管理员+饲养员+游客：显示某园区环境
...
class Permission_Manage{
    protected:
    vector<Animal_Manage*> areas;

    public:
    Permission_Manage(const vector<Animal_Manage*>& areas);//通过这种方式来操作Amimal_Manage的派生类
    ~Permission_Manage();
...
class Admin:public Permission_Manage{
    ...
    Tourism_Manage& tourism_manage;//引用一个对象tourism_manage

    public:
    bool login();//对于管理员，登陆提示输入id和passwd，进行验证
    void add_feeder(const string& name,int id,int passwd,const vector<Staff>& keeper);//添加新的饲养员
    void add_admin(const string& name,int id,int passwd);//添加新的管理员
...
class Keeper:public Permission_Manage{
    ...
    void health_repo();//更新自己管理动物的健康状况（以种群为单位）
    void feed_repo();//更新自己管理动物的饲喂状况（以种群为单位）
};
...
/*Tourist_Service类负责满足游客的各种需求，包括购票，搜索某种动物
...
class Tourist_Service{
    ...
    public:
    Tourist_Service(vector<Animal_Manage*>& areas);
